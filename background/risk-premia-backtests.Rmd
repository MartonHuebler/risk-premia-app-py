...
- capFrequency <- 1 # frequence to capitalise profits. 0 is "don't capitalise"
- assetVolTarget <- 0.05 
- volLookback <- 60 
...
endDate <-backtest_end_date  # monthlyprices %>% summarise(max(date)) %>% pull()
...
```
## Scale to 10% vol based on recent realised vol
...
port <- volsize_rebal %>% 
  ...
  mutate(returns = (nav - lag(nav, 1))/lag(nav, 1))

# scale to vtarget based on realised recent vol
# port_vol_lookback <- 4  # MONTHS!!!
# vtarget <- 0.1
# port <- port %>% 
#   mutate(scaled_returns = vtarget / (runSD(returns, n = port_vol_lookback) * sqrt(12)) * returns)

# scale up based on actual vol - this is a bit of a cheat but ok for demo
realised_vol <- port %>% 
  summarise(realised_vol = sd(returns, na.rm = TRUE)*sqrt(12)) %>% 
  pull()

port <- port %>% 
  mutate(scaled_returns = returns * 0.1/realised_vol)


port %>% 
  summarise(realised_vol = sd(scaled_returns, na.rm = TRUE)*sqrt(12))

port %>% 
  ggplot(aes(x = date, y = runSD(scaled_returns, n = port_vol_lookback) * sqrt(12))) +
    geom_line() +
    geom_hline(yintercept = vtarget)
...
```{r}
port %>% 
  select(date, scaled_returns) %>% 
  saveRDS(file = here::here("portfolio", "data", "rp_returns_volten.RDS"))
```
